...
When a skill bundles more than one script, list them in the body before first use so the agent knows they all exist — a single script is introduced by its own invocation. Use relative paths from the skill directory root, both in listings and in code block invocations.
