...
Check whether the venv is already set up:
...
**CUDA check** — run this whether or not the venv was already set up:
...
`<source>` is the URL or local file path. The script handles download (for URLs), transcription, and temp file cleanup automatically.
