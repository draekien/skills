...
Load libraries via CDN — do not bundle or download anything. `scripts/validate.py` only accepts resources from its `ALLOWED_CDN_HOSTS` list (currently `cdn.tailwindcss.com`, `cdnjs.cloudflare.com`, `cdn.jsdelivr.net`, `cdn.plot.ly`) — check that list before using an unlisted host. Approved CDN libraries:
