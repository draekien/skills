...
# ///
"""
Reads and writes the module-design section of .draekien/.skillsrc.

Usage:
  uv run scripts/skillsrc.py --config <path> get
  uv run scripts/skillsrc.py --config <path> set <value>

get  — prints specsDir value (or default "docs/designs" if absent)
set  — merges specsDir into the file, preserving all other skills' keys
...
def main() -> int:
    ...
    set_p.add_argument("value", help="New value for specsDir")
