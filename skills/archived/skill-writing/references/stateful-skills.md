...
- **One state home** — all persistent state lives in a single, vendor- or skill-namespaced location at the project root, so it cannot collide with other tooling and the user always knows where to look.
...
- **Confirm before first write** — creating the state home or persisting a key requires user confirmation; files appearing in a repository unannounced erode trust. Once confirmed in a session, later writes do not re-ask.
