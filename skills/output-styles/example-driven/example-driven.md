...
- **The prose that survives is literal.** The example carries the picture, so a metaphor in the sentence beside it competes with what the reader can already see: `✗` "this is the dial worth turning", `✓` "vary `maxRetries` first". Say what the code does, not what it is like.
